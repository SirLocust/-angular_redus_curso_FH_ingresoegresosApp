...
export class IngresoEgresoService {
  ...
  initIngresoEgresoListener(){
   ...
   this.ingreosEgresoListenerSub = this.store.select('auth')
  ...
  private ingresoEgresoItems( uid:string){
   this.ingresoEgresoItemsSub = this.afDB.collection(`${uid}/ingresos-egresos/items`).snapshotChanges()
      .pipe(
        map( docData => {
  ...
  crearIngresoEgreso( ingresoEgreso:IngresoEgreso):Promise<any>{
    ...
    return this.afDB.doc(`${this.authService.getUsuario().getUID()}/ingresos-egresos`)
    .collection('items').add(JSON.parse(JSON.stringify(ingresoEgreso)))
